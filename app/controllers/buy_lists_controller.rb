class BuyListsController < ApplicationController
  ...
  def update_all_prices
    ...
    cards.each do |card|
      ...
      if !doc.at_css('.clearfix table.prod-variants .even td').blank?
        card.condition = doc.at_css('.clearfix table.prod-variants .even td').text
        card.price = doc.at_css('.clearfix table.prod-variants .even td .price').text[/[0-9\.]+/]
        card.stock = doc.at_css('.clearfix table.prod-variants .even td .qty').text[/[0-9\.]+/]
      else
        card.stock = nil
        card.price = 0
        card.condition = nil
